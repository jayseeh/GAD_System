...
class myPDF extends FPDF {
	function header(){
		...
		$this->Ln(10);
		$this->SetFont('Times','B',15);
		$this->Cell(276,10,'ANNUAL GENDER AND DEVELOPMENT (GAD) PLAN AND BUDGET',0,0,'C');
		$this->Ln(20);
	}
	...
	function headerTable(){
		...
		$this->Cell(28,15,'Output Performance Indicator/ Target',1,0,'C');
		$this->Cell(28,15,'GAD Budget',1,0,'C');
		$this->Cell(28,15,'Source of Budget',1,0,'C');
		$this->Cell(28,15,'Responsible Unit/ Office',1,0,'C');
		$this->Ln();
	}
	function viewTable(){
		$this->setFont('Times','',8);
		...
		$this->Cell(252,10,'CLIENT FOCUSED',1,0,'L');
		$this->Ln();
		...
		$query1 = mysqli_query($conn,"SELECT * FROM gad_form INNER JOIN gad_table_entry_value ON gad_form.form_number=gad_table_entry_value.form_number WHERE gad_form.form_status='APPROVED' AND gad_table_entry_value.category_focused='ORGANIZATION' AND gad_form.form_number LIKE '%GPB%' ORDER BY gad_form.form_number"); 
		$this->Cell(252,10,'ORGANIZATION FOCUSED',1,0,'L');
		$this->Ln();
